...
import { packBraille } from './braille';
...
function foregroundWaterChar(
	...
	const sample = sampleOceanSurface(centerSx, waterT, params.waterTime, config.subWidth, config.subHeight, waveParams);
...
export function generateHorizon(
	...
	for (let y = 0; y < config.height; y++) {
		...
		for (let x = 0; x < config.width; x++) {
			...
			} else if (zone === 'water-far' || zone === 'water-reflect' || zone === 'water-reflect-warm' || zone === 'water-reflect-cool') {
				char = waterBraille[y][x].char;
			} else if (zone === 'sun-core') {
				char = '█';
			} else {
	...
	for (let y = sunMirrorTop; y < sunMirrorBottom; y++) {
		for (let dx = 1; dx <= sunRadiusCx; dx++) {
			...
			const sunZones = new Set(['sun-core', 'sun', 'sky-glow']);
			if (!sunZones.has(leftZone) && !sunZones.has(rightZone)) continue;
