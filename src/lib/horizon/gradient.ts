		...
		for (let x = 0; x < subWidth; x++) {
			...
			if (y < horizonRow) {
				...
				if (dist < 3.0) {
					...
					if (dist < 1.0) {
						// Solid core — dithers to full-block Braille
						sunBrightness = 255;
					} else {
						// Smooth quadratic falloff from dist 1.0 → 2.2
						const t = Math.min((dist - 1.0) / 1.2, 1);
			...
			} else if (y >= horizonRow && y < horizonRow + 8) {
				// --- Horizon band --- blends from horizon glow into water+reflection
				const bandT = (y - horizonRow) / 8;
				const dxNorm = (x - sunCenterX) / subWidth;
				const gauss = Math.exp(-(dxNorm * dxNorm) / (2 * 0.22 * 0.22));
				const peak = 200 + params.horizonGlow * 40;
				const horizonBright = 105 + (peak - 105) * gauss;
				...
				const horizonReflect = 64 * Math.max(0.3, params.horizonGlow) * Math.max(reflection.reflectScore, gauss * 0.28);
				const waterBright = 152 + 8 * sample.crest + horizonReflect;
