...
function render() {
	if (!state.ctx || !state.config || !state.metrics || !state.palette || !state.waveParams || !state.skyParams) {
		return;
	}

	const waterTime = getWaterTime(performance.now());
	const grid = generateHorizon(Date.now(), state.config, waterTime, state.waveParams, state.skyParams);
